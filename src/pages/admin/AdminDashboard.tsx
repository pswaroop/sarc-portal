...
export default function AdminDashboard() {
  ...
  const openTickets = tickets.filter((t) => t.state === "New" || t.state === "In Progress" || t.state === "On Hold").length;
  const portfolioWorth = useMemo(
    () => financials.reduce((sum, f) => sum + (Number(f.project_worth) || 0), 0),
    [financials]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          label="Portfolio"
          value={`${currency} ${(portfolioWorth / 1000).toFixed(0)}k`}
          icon={DollarSign}
          variant="success"
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            {projects.map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-2 rounded-lg border bg-card/40 px-3 py-2 text-sm">
