...
function getProjectTypeMultiplier(type?: string | null) {
  if (!type) return 1.0;
  const t = type.toLowerCase();
  if (t.includes("fullstack") || t.includes("full stack")) return 1.5;
  if (t.includes("backend") || t.includes("back end")) return 1.2;
  return 1.0; // Default for Frontend, UI/UX, etc.
}
...
export default function ManagerPerformance() {
  ...
  return proj?.status !== "Backlog";
        ...
        empAssign.forEach(a => {
          const proj = projects.find(p => p.id === a.project_id);
          const alloc = a.allocation_percentage || 100;
          const mult = getProjectTypeMultiplier(proj?.project_type);
          const weight = alloc * mult; // Weighted by Allocation & Project Complexity

          const expected = elapsedPercent(a.start_date, a.end_date);
          const actual = a.completion_percentage;
          const variance = actual - expected;

          let assignScore = 100;
          if (actual >= 100) {
            assignScore = 100;
            // Early completion bonus if finished before deadline
            if (a.end_date && new Date(a.end_date) > now) {
              assignScore = 110; 
            }
          } else if (variance < 0) {
            // Deduct points from 100 based on negative variance
            assignScore = Math.max(0, 100 + variance);
          }

          weightedDeliveryScoreSum += (assignScore * weight);
          totalWeight += weight;
        });

        // 1. Delivery Score (Weighted Average)
        const deliveryScore = totalWeight > 0 ? Math.min(100, weightedDeliveryScoreSum / totalWeight) : 100;

        // 2. Ticket Score (Penalize ONLY unattended Critical/High bugs, stop punishing overall volume)
        const empTickets = tickets.filter(t => t.assigned_to === emp.id && (!interval || isWithinInterval(new Date(t.updated_at || t.created_at), interval)));
        const openCriticalHigh = empTickets.filter(t => !["Resolved", "Closed"].includes(t.state) && ["Critical", "High"].includes(t.priority)).length;
        const ticketsResolved = empTickets.filter((t) => t.state === "Resolved" || t.state === "Closed").length;
        const ticketsOpen = empTickets.filter((t) => !["Resolved", "Closed"].includes(t.state)).length;
        const ticketScore = Math.max(0, 100 - (openCriticalHigh * 10));

        // 3. Engagement / Updates Score
        const empUpdates = updates.filter(u => u.employee_id === emp.id && (!interval || isWithinInterval(new Date(u.date), interval)));
        const updateCount = empUpdates.length;
        const blockers = empUpdates.filter((u) => u.has_blocker).length;
        const expectedUpdates = period === "week" ? 5 : period === "month" ? 20 : 50;
        const updateScore = Math.min(100, (updateCount / expectedUpdates) * 100);

        // 4. Volume / Throughput Metric (Based on active and completed load)
        const completedAssigns = empAssign.filter(a => a.completion_percentage >= 100).length;
        const activeAssigns = empAssign.length - completedAssigns;
        const activeProjectsList = empAssign.filter(a => a.completion_percentage < 100).map(a => projects.find(p => p.id === a.project_id)?.name || "Unknown");
        const volumeScore = (activeAssigns > 0 || completedAssigns > 0) ? 100 : 0;

        // Final Composite Score (Delivery: 50%, Updates: 20%, Tickets: 15%, Workload Volume: 15%)
        let score = Math.round((deliveryScore * 0.50) + (updateScore * 0.20) + (ticketScore * 0.15) + (volumeScore * 0.15));

        // For display charts (unweighted raw averages)
        const expected = totalWeight > 0 ? Math.round(empAssign.reduce((s,a) => s + elapsedPercent(a.start_date, a.end_date), 0)/empAssign.length) : 0;
        const actual = totalWeight > 0 ? Math.round(empAssign.reduce((s,a) => s + a.completion_percentage, 0)/empAssign.length) : 0;
        const variance = actual - expected;
  ...
  const healthDist = useMemo(() => {
    return [
      { name: "On track / Ahead", value: kpis.onTrack, fill: "hsl(var(--success))" },
      { name: "Slightly Behind", value: kpis.behind, fill: "hsl(var(--warning))" },
      { name: "Lagging (>20%)", value: kpis.lagging, fill: "hsl(var(--destructive))" },
    ].filter((d) => d.value > 0);
  }, [kpis]);
  ...
  const deadlineWatchlist = useMemo(() => {
    ...
    return scoped
      ...
      .map((a) => {
        ...
        const isApproaching = diff >= 0 && diff <= 7;
        ...
        return {
          ...
          employeeName: employees.find((e) => e.id === a.employee_id)?.full_name || "Unknown",
          endDate: a.end_date,
  ...
  }, [scoped, projects, employees, teamMembers]);
  ...
  const handleExportPDF = () => setTimeout(() => window.print(), 100);
  ...
  return (
    <div className="space-y-6 max-w-full overflow-hidden px-1 sm:px-0 print:bg-white print:text-black print:p-8 print:w-full print:m-0">
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 print:grid-cols-4 print:gap-4 print:break-inside-avoid">
        ...
        <KpiCard label="Completed" value={kpis.completedTasks} icon={Briefcase} variant="success" />
        <KpiCard label="Tickets Fixed" value={kpis.totalResolved} icon={CheckCircle2} variant="info" />
        <KpiCard label="Blockers Logged" value={kpis.totalBlockers} icon={Activity} variant="warning" />
      </div>

      {/* --- DEADLINE WATCHLIST & LEADERBOARD --- */}
      <div className="grid gap-6 lg:grid-cols-3 print:grid-cols-2 print:gap-4 print:break-inside-avoid">
        
        {/* Deadline Watchlist */}
        <Card className="shadow-sm lg:col-span-2 print:shadow-none print:border-gray-200">
          <CardHeader className="pb-2 border-b bg-muted/10 print:bg-gray-50 print:border-gray-200">
            <h2 className="flex items-center gap-2 text-base font-semibold print:text-black">
              <CalendarClock className="h-4 w-4 text-primary print:text-gray-800" /> Deadline Watchlist
            </h2>
            <p className="text-xs text-muted-foreground print:text-gray-600">Projects overdue or due within 7 days.</p>
          </CardHeader>
          <CardContent className="p-0 max-h-[300px] overflow-y-auto">
            <div className="divide-y print:divide-gray-200">
              {deadlineWatchlist.length === 0 ? (
                <div className="p-6 text-center text-sm text-muted-foreground print:text-gray-600">All assigned projects are safely within deadlines.</div>
              ) : (
                deadlineWatchlist.map((dw: any) => (
                  <div key={dw.id} className={`p-4 flex items-center justify-between gap-3 ${dw.status === "Overdue" ? "bg-destructive/5" : ""}`}>
                    <div className="min-w-0">
                      <div className="font-semibold text-sm truncate flex items-center gap-2">
                        {dw.status === "Overdue" && <AlertCircle className="w-3.5 h-3.5 text-destructive" />}
                        {dw.projectName}
                      </div>
                      <div className="text-xs text-muted-foreground mt-0.5">Assigned to: <span className="font-medium">{dw.employeeName}</span></div>
                    </div>
                    <div className="shrink-0 text-right flex flex-col items-end gap-1">
                      <StatusBadge label={dw.status === "Overdue" ? `Overdue by ${Math.abs(dw.daysDiff)} days` : `Due in ${dw.daysDiff} days`} variant={dw.status === "Overdue" ? "destructive" : "warning"} />
                      <span className="text-[10px] font-bold text-muted-foreground">{dw.completion}% Complete</span>
                    </div>
                  </div>
                ))
              )}
        ...
        {selectedEmployee === "all" ? (
          <Card className="shadow-sm print:shadow-none print:border-gray-200">
            <CardHeader className="pb-0 border-b bg-muted/10 print:bg-gray-50 print:border-gray-200">
              <h2 className="flex items-center gap-2 text-base font-semibold mb-2 print:text-black">
                <Award className="h-4 w-4 text-primary print:text-gray-800" /> Leaderboards
              </h2>
            </CardHeader>
            <CardContent className="p-0">
              <div className="print:hidden">
          ...
          <Card className="shadow-sm flex flex-col items-center justify-center p-8 bg-muted/10 print:hidden">
            <Award className="h-12 w-12 text-primary/40 mb-3" />
            <h3 className="font-semibold text-lg">Individual View Active</h3>
            <p className="text-sm text-muted-foreground">Clear filter to view leaderboards.</p>
          </Card>
        )}
      </div>
      ...
      <Card className="shadow-sm print:shadow-none print:border-gray-200 print:break-inside-avoid">
        <CardHeader className="pb-2 bg-muted/10 border-b print:bg-transparent print:border-b-2 print:border-black print:px-0">
          <CardTitle className="text-lg font-bold print:text-black">Employee Workload & Active Projects</CardTitle>
          <p className="text-sm text-muted-foreground print:text-gray-600">Overview of current and completed assignments.</p>
        </CardHeader>
        <CardContent className="p-0 print:px-0">
          <Table>
            ...
            <TableBody>
              {allTimeMetrics.length === 0 && (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-6 text-muted-foreground">No workload data found.</TableCell>
                </TableRow>
              )}
              {allTimeMetrics.map((emp) => (
                <TableRow key={emp.id}>
                  <TableCell className="font-semibold">{emp.full_name}</TableCell>
                  <TableCell className="text-center font-medium">{emp.activeAssigns}</TableCell>
                  <TableCell className="text-center text-success font-medium">{emp.completedAssigns}</TableCell>
                  <TableCell className="text-xs text-muted-foreground max-w-xs">
                    {emp.activeProjectsList.length > 0 
                      ? emp.activeProjectsList.join(", ") 
                      : "No active projects"}
                  </TableCell>
                </TableRow>
              ))}
...
function LeaderboardList({ metrics }: { metrics: any[] }) {
  const topPerformers = metrics.slice(0, 5);
  if (topPerformers.length === 0) return <div className="text-sm text-muted-foreground p-8 text-center print:text-gray-600">No active data.</div>;
      ...
      {topPerformers.map((p, i) => (
        <div key={p.id} className="flex items-center gap-3 py-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 h-1 overflow-hidden rounded-full bg-muted/60 dark:bg-muted print:hidden">
              <div className={`h-full rounded-full transition-all ${i === 0 ? 'bg-yellow-500' : 'bg-primary'}`} style={{ width: `${p.score}%` }} />
            </div>
          </div>
        </div>
      ))}
